...
def main():
    ...
    if len(sys.argv) >= 3 and sys.argv[1] == "split_to_cache" and sys.argv[2].isdigit() and (sys.argv[3] in ["train", "test", "val"]):
        train_test_val = sys.argv[3]
        fdp = ForecastingDatasetProcessor(train_test_val=train_test_val)
        ...
        print(f"Generating {train_test_val} cache from {desired_range[0]} to {desired_range[-1]}")
        fdp.generate_network_data(dev_mode=False, save_to_pickle=True, desired_range=desired_range)

    elif len(sys.argv) >= 3 and sys.argv[1] == "merge_cache" and (sys.argv[2] in ["train", "test", "val"]):
        train_test_val = sys.argv[2]
        print(f"Merging cache {train_test_val}")
        fdp = ForecastingDatasetProcessor(train_test_val=train_test_val)
        fdp.merge_cache()
    elif len(sys.argv) < 2:
        desired_range = range(1, 10)
        print(f"Generating train cache from {desired_range[0]} to {desired_range[-1]}")

        fdp = ForecastingDatasetProcessor(train_test_val="train")
        scene_input, social_input, person_input, expected_output = fdp.generate_network_data(dev_mode=False, save_to_pickle=True, desired_range=desired_range)
    else:
